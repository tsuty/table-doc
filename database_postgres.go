...
func (config *PostgresConfig) Databese() (Database, error) {
	db, err := sql.Open("postgres", config.String())
	if err != nil {
		return nil, errors.New(fmt.Sprintf("PostgresConfig#Databese: fail to open postgres connection. %s", err.Error()))
	}
